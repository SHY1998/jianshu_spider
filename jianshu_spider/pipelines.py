...
class JianShuTwistedPipeline(object):
    ...
    def sql(self):
        if not self._sql:
            self._sql = """
                        insert into article(id,title,content,author,avatar,pub_time,origin_url,article_id,read_count,like_count,word_count,subjects) values(null,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s)
                        """
            return self._sql
        return self._sql
